...
static void sec_req_timeout_handler(void * p_context)
{
    ...
    if (m_peer_handle.connection_id != DM_INVALID_ID)
    {
		...
		//NRF_LOG("dm_security_status_req is OK\r\n");
...
static void timers_init(void)
{
    ...
    APP_ERROR_CHECK(err_code);
	NRF_LOG_PRINTF("timers_init err_code = %d\r\n", err_code);
}
...
static void on_ancs_c_evt(ble_ancs_c_evt_t * p_evt)
{
    ...
    {
        case BLE_ANCS_C_EVT_DISCOVERY_COMPLETE:	//连接成功 建立了成功的连接，并在连接的对等方上找到了服务
            ...
            APP_ERROR_CHECK(err_code);
            apple_notification_setup();	//设置通知使能
            break;

        case BLE_ANCS_C_EVT_NOTIF:	//接收到IOS通知时所做的处理,获取详细的通知信息 在通知源控制点上收到iOS通知
            m_notification_latest = p_evt->notif;
            notif_print(&m_notification_latest);
            break;

        case BLE_ANCS_C_EVT_NOTIF_ATTRIBUTE:	//收到的iOS通知属性已解析
            notif_attr_print(&p_evt->attr);
            break;

        case BLE_ANCS_C_EVT_DISCOVERY_FAILED:	//无法发现所连接对等方的服务或特征
...
static void gap_params_init(void)
{
    ...
    err_code = sd_ble_gap_device_name_set(&sec_mode,
                                          ...
                                          strlen(DEVICE_NAME));
    APP_ERROR_CHECK(err_code);
	NRF_LOG_PRINTF("sd_ble_gap_device_name_set err_code = %d\r\n", err_code);
    ...
    APP_ERROR_CHECK(err_code);
	NRF_LOG_PRINTF("sd_ble_gap_ppcp_set err_code = %d\r\n", err_code);
}
...
{
	NRF_LOG("apple_notification_error_handler\n\r");
...
{
	NRF_LOG("db_disc_handler ble_ancs_c_on_db_disc_evt\n\r");
                                           ...
                                           ret_code_t          event_result)
{
    ...
    {
        case DM_EVT_CONNECTION:	//判断设备是否连接 手机确定匹配后立即触发的设备连接中断位置
            m_peer_handle = (*p_handle);
			NRF_LOG("DM_EVT_CONNECTION Connected.\n\r");
            err_code      = app_timer_start(m_sec_req_timer_id, SECURITY_REQUEST_DELAY, NULL);
            APP_ERROR_CHECK(err_code);
            break;

        case DM_EVT_LINK_SECURED:	//表示与对等方的链接是安全的
			NRF_LOG("DM_EVT_LINK_SECURED Connected.\n\r");
            err_code = ble_db_discovery_start(&m_ble_db_discovery,
                                              p_evt->event_param.p_gap_param->conn_handle);
            APP_ERROR_CHECK(err_code);
		
			err_code = ble_ancs_c_request_attrs(&m_ancs_c, &m_notification_latest);	//ANCS开启请求通知属性的功能
            APP_ERROR_CHECK(err_code);
...
static void device_manager_init(bool erase_bonds)
{
    ...
    APP_ERROR_CHECK(err_code);
	NRF_LOG_PRINTF("pstorage_init err_code = %d\r\n", err_code);

    err_code = dm_init(&init_param);
    APP_ERROR_CHECK(err_code);
	NRF_LOG_PRINTF("dm_init err_code = %d\r\n", err_code);
    ...
    APP_ERROR_CHECK(err_code);
	NRF_LOG_PRINTF("device_manager_init err_code = %d\r\n", err_code);
}
...
static void on_ble_evt(ble_evt_t * p_ble_evt)
{
    ...
    {
        case BLE_GAP_EVT_CONNECTED:{	//与对等体连接
            NRF_LOG("Connected.\n\r");
        ...
        case BLE_GAP_EVT_DISCONNECTED:	//与对等体断开连接
...
static void ble_stack_init(void)
{
    ...
    err_code = softdevice_enable_get_default_config(CENTRAL_LINK_COUNT,
                                                    ...
                                                    &ble_enable_params);
    APP_ERROR_CHECK(err_code);
	NRF_LOG_PRINTF("softdevice_enable_get_default_config err_code = %d\r\n", err_code);
    ...
    APP_ERROR_CHECK(err_code);
	NRF_LOG_PRINTF("softdevice_enable err_code = %d\r\n", err_code);

    // Register with the SoftDevice handler module for BLE events.
    err_code = softdevice_ble_evt_handler_set(ble_evt_dispatch);
    APP_ERROR_CHECK(err_code);
	NRF_LOG_PRINTF("softdevice_ble_evt_handler_set err_code = %d\r\n", err_code);

    // Register with the SoftDevice handler module for System events.
    err_code = softdevice_sys_evt_handler_set(sys_evt_dispatch);
    APP_ERROR_CHECK(err_code);
	NRF_LOG_PRINTF("ble_stack_init err_code = %d\r\n", err_code);
...
static void service_init(void)
{
    ...
    err_code = ble_ancs_c_attr_add(&m_ancs_c,
                                   ...
                                   ATTR_DATA_SIZE);
    APP_ERROR_CHECK(err_code);
	NRF_LOG_PRINTF("ble_ancs_c_attr_add err_code = %d\r\n", err_code);
    ...
    APP_ERROR_CHECK(err_code);
	NRF_LOG_PRINTF("service_init err_code = %d\r\n", err_code);
}
...
static void buttons_leds_init(bool * p_erase_bonds)
{
    ...
    APP_ERROR_CHECK(err_code);
	NRF_LOG_PRINTF("bsp_init err_code = %d\r\n", err_code);

    err_code = bsp_btn_ble_init(NULL, &startup_event);
    APP_ERROR_CHECK(err_code);

    *p_erase_bonds = (startup_event == BSP_EVENT_CLEAR_BONDING_DATA);
	NRF_LOG_PRINTF("buttons_leds_init err_code = %d\r\n", err_code);
}
...
static void db_discovery_init(void)
{
    ...
    APP_ERROR_CHECK(err_code);
	NRF_LOG_PRINTF("ble_db_discovery_init err_code = %d\r\n", err_code);
}
...
static void nrf_log_init(void)
{
    ...
    APP_ERROR_CHECK(err_code);
	NRF_LOG_PRINTF("nrf_log_init err_code = %d\r\n", err_code);
}
...
int main(void)
{
    ...
    // Initialize.
    nrf_log_init();	//调试信息LOG初始化
	timers_init();	//定时器初始化
//    buttons_leds_init(&erase_bonds);	//按钮与LED灯初始化
...
//    scheduler_init();
    ...
    for (;;)
    {
        ...
        power_manage();	//电源管理	
		if(flag){
			ble_ancs_c_request_attrs(&m_ancs_c, &m_notification_latest);	//请求通知属性的功能
		}
		nrf_delay_ms(100);
    }
